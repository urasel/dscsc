<?php
    ...
    class LineChart extends BarChart {
         ...
         */
        public function LineChart($width = 1200, $height = 700) {
            parent::BarChart($width, $height);

            $this->plot->setGraphPadding(new Padding(5, 30, 150, 50));
        }
            ...
            for ($j = 0; $j < count($serieList); $j++) {
                ...
                for ($i = 0; $i < $pointCount; $i++) {
					...
					if ($x1) {
						...
						$test['blue']=36;
						$obj2=new Color();
						$sscolor=$obj2->Color(255,0,36,0);
				
						if($value==63){
							$primitive->line(55, $y2, $x2, $y2, $sscolor, 1);
						}else{
							$primitive->line($x1, $y1, $x2, $y2, $lineColor, 2);
						}//else 
