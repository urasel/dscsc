...
switch($action){
	...
	default:
		...
		if($term_id != 0){
			...
			$sql = " SELECT distinct (SUM(mrk.ds_student_weight+mrk.si_student_weight+mrk.ci_student_weight)) as total, mrk.student_id from ".DB_PREFIX."marking as mrk WHERE mrk.course_id = '".$course_id."' AND mrk.status = 1 AND mrk.ci_status = '1' AND (";			
			if(!empty($termArr)){
...
require_once("templates/left_menu.php");
		...
		//echo '<pre>';
		//print_r($numberArrayForTotalTerm);exit;
						...
						<table cellpadding="0" cellspacing="0" border="0" class="datagrid">
							...
							<tr <?php echo $class; ?>>
								...
								foreach($termArr as $term){
									...
									//$sql = "select exr.* from ".DB_PREFIX."exercise as exr,  ".DB_PREFIX."exercise_to_term as ett WHERE ett.exercise_id = exr.id AND ett.term_id = '".$term->id."'";
									
									//$sql = "select exr.id,exr.name, exr.mark, exr.weight from ".DB_PREFIX."exercise as exr,  ".DB_PREFIX."exercise_to_term as ett WHERE exr.course_id = '".$course_id."' AND ett.exercise_id = exr.id AND ett.term_id = '".$term->id."'";
								...
								//New Array Create For Position Calculation End
								...
								//echo 'Exr wgt= '.$exercise_weight.'<br/>';
								...
								<td align="center">
									<?php 
									//echo '<pre>';
									//print_r($termWiseArray[$term->id]);
									//print_r($std_term_total_weight);
									//echo findPosition($numberArrayForTerm, $std_term_total_weight);
									echo findPosition($termWiseArray[$term->id], $std_term_total_weight);
											$arr[$sl][$term->id.'_posn'] = findPosition($termWiseArray[$term->id], $std_term_total_weight);
									?>
								</td>
								<?php 
								$str_pad .= '
									...
									<td align="center">'.findPosition($termWiseArray[$term->id], $std_term_total_weight).'</td>';
									}//foreach -- termArr
								if($term_id == '0' || empty($term_id)){
								...
								$str_pad .= '
									...
									<td align="center">'.findPosition($numberArrayForTotalTerm, $student_total_weight).'</td>
								</tr>';
									}else{ ?>
								<td align="center">
									<?php echo view_number_two($term_total_weight); 
								...
								<td align="center">
									<?php 
									//echo $term_total_weight;exit;
									//echo findPosition($numberArrayForTermAggregate, $term_total_weight);
									echo findPosition($numberArrayForTotalTerm, $term_total_weight);
											...
											$arr[$sl]['position'] = findPosition($numberArrayForTotalTerm, $term_total_weight); ?>
								</td>
								<?php }//else ?>
							</tr>
							<?php	
