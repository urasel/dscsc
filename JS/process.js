...
{	
	...
	switch(actiontype){
		...
		case 'assign_ds_mark':
			...
			var term_id = document.getElementById("term_id").value;
			var syndicate_id = document.getElementById("syndicate_id").value;
			fileToProcess = "ajax.php?action=assign_ds_mark&exercise_id="+exercise_id+"&syndicate_id="+syndicate_id+"&term_id="+term_id;	
		break;
		...
		case 'syndicate_wise_exercise':
			var syndicate_id = document.getElementById("syndicate_id").value;
			var term_id = document.getElementById("term_id").value;
			fileToProcess = "ajax.php?action=syndicate_wise_exercise&syndicate_id="+syndicate_id+"&term_id="+term_id;
		break;
	...
	xmlhttp.onreadystatechange=function(){
		...
		}else if(action == 'syndicate_wise_exercise'){
			if(xmlhttp.readyState!=4){
				document.getElementById("loaderContainer").style.display = "block";
...
function validateSiImprMarking(id){
	...
	if(num == '' || isNaN(num) || num > highest || (length == 2 && num < parseInt(lowest) && num > 10) || (length == 1 && num < parseInt(lowest) && num.length == 2)){
		if(isNaN(num)){
			alert("Only Number Supported!");
		}else if(num > highest){
			alert("The highest value you can put here is "+highest);
		}else if(length == 2 && num < parseInt(lowest) && num > 10){		//10 is for inserting 100
			alert("The lowest value you can put here is "+lowest);
		}else if(length == 1 && num < parseInt(lowest) && num.length == 2){
			alert("The lowest value you can put here is "+lowest);
		}//else
		if(num != ''){
			continue_flag = 1;
...
function validateStudentImprOfSI(){
	var put_number, prev_number, total_count, student_id;
	...
	for(i=1;i < total_count;i++){
		...
		student_id = document.getElementById('student_id_'+i).innerHTML;
		
		if((put_number < lowest) && (put_number > 0)){
			alert('The lowest value you can put for C/N '+student_id+' is '+lowest);
			return false;
		}
	}
}//function 
